...
\begin{listing}[language = rust, caption = {Example of Tokio usage}]
...
The two functions run_server and client_test are both async functions. The server function is a simple echo server, that waits for a message from the client and then sends it back. The client function sends a message containing an url and a vote to the server and then waits for the response. Server and Client are connected via a gRPC connection. 
...
\begin{listing}[language = rust, caption = {Example Protocol Buffer for Voting Service}]
    syntax = "proto3";
    package voting;
