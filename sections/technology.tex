...
\begin{enumerate}
    ...
    \item \textbf{Enum:} %Define Enum in protobuf
...
and then assert if the returned data equals true or false according to the provieded boolean operator. If it does then it was successful and the rest of the \textit{TEST\_CASE} is executed. The difference 
...
The benchmarking macros in Catch2 are defined similarly to how unit tests are. Benchmarking in itself is a useful practice, that provides a way to mesure the performance
...
is first tested wether or not it actually works as intended before any benchmarks are done. This is done with the REQUIRE macro, since it throws an execption if the 
...
After this is run a summary is automatically output within the commandline window. This includes multiple datapoints that pertain to the execution speed of the tested
function:
\begin{enumerate}
