...
database environments, fostering ease of testing, development, and even benchmarking. In the specific use case of RECT, Docker proved instrumental in benchmarking 
different databases, allowing us to evaluate and identify the most suitable database for the application's requirements.\newline
...
In summary, Docker's role in the project extends beyond conventional application development and deployment. It serves as a significant tool for benchmarking 
databases. Its containerization approach, coupled with the ability to create reproducible environments, positions Docker as a valuable asset in order to 
identify the optimal database solution for the RECT use case. The open-source nature and cross-platform compatibility further solidify Docker's standing as 
...
The decision to adopt rusqlite for the use case is underpinned by a meticulous consideration of three critical factors:
...
\begin{enumerate}
    ...
    the context of RECT. Given the heterogeneous nature of small controllers operating within the RECT system, the ability to seamlessly deploy SQLite databases 
    ...
    The simplicity inherent in working with SQLite fits perfectly with the operational requirements. Since we need to configure a database on each controller, 
    SQLite is the optimal choice for the lean operating requirements of RECT due to its low setup effort.
...
In the instantiation of RECT, rusqlite is strategically employed for the persistent storage of the config.json file, a repository of vital data concerning 
...
enhanced performance of the system but also underscores the commitment to preserving the integrity and reliability of configuration data, critical to the seamless
functionality of the application within the broader scientific and technological landscape.
...
simplicity. This functionality proves invaluable in the context of RECT, where the utilization of data from the config.json file is an integral part of the program. 
Serde streamlines this process, enabling us to incorporate the data effortlessly into the program with just a few lines of code.\newline
...
structure. This bespoke structure, tailored to the specific needs of the project, enables us to harness the deserialized data seamlessly within the program. The utilization of
...
In essence, the incorporation of Serde into the workflow of the RECT is not merely a technical choice but a strategic one, driven by the need for optimized data handling 
and seamless integration of external data sources. Through Serde, the program gains a robust and flexible capability to decode JSON files, thereby enhancing the
overall efficiency and maintainability of the Rust-based application.
...
The selection of Tokio as asynchronous runtime is underpinned by several compelling reasons. Chief among them is Tokio's status as the preeminent and widely
...
In summary, the decision to embrace Tokio as the asynchronous runtime for the Rust project is grounded in its status as the leading runtime in the Rust 
...
full potential of asynchronous programming in Rust, ensuring the responsiveness and scalability of the application.
...
This is done using the tokio::select! macro. The macro takes a list of futures and waits for the first one to finish. In the case of RECT we want to wait for the client to finish 
...
The seamless integration of Tokio Rusqlite into the development stack of RECT augments the versatility of Rusqlite by enabling asynchronous database operations. This is 
...
while the backend is running. In this case the backend can continue to run without having to wait for the database to finish updating. Leveraging Tokio Rusqlite in the project 
...
The utilization of Tonic in the project streamlines the definition of interfaces by leveraging Protocol Buffers. This enables us to articulate the interface 
...
In essence, Tonic serves as a powerful tool in the technology stack, providing a performant and flexible implementation of gRPC for Rust. Its compatibility with 
...
choosing Tonic, we aim to leverage its capabilities to enhance the performance, interoperability, and maintainability of the Rust-based projects.\newline
